...
export default class Overview extends Component {
    ...
    try {
      ...
      if (value !== null) {
        ...
        if (datastring.length > 0) {
          ...
          this.getDataByUsia(datastring[0]['k_id']);
          this.GetDataPendukung(datastring[0]['k_id']);
          this.getDataHarianRekrut(datastring[0]['k_id']);
          this.GetDataDapil(datastring[0]['k_id']);
          this.GetDataByGender(
            datastring[0]['k_id'],
            datastring[0]['dapil_id'],
          );
        }
      }
    } catch (e) {
  ...
  GetDataPendukung = idkandidat => {
    ...
    if (this.state.ishead === 1) {
      urlapi = API_URL + `/totalpendukunghead/` + idkandidat;
    } else {
      urlapi = API_URL + `/totalpendukung/` + idkandidat;
    }
    const {navigation} = this.props;
    ...
    axios(config)
      .then(function (response) {
        ...
        if (response.data.success === true) {
          ...
          //console.log("DDDDDAAAAAATTTTTTTTTAAAAAA",(dataRes[0].total_pendukung/dataRes[0].target));
          self.setState({
            target: dataRes[0].target,
            total_relawan: dataRes[0].total_relawan,
            total_pendukung: dataRes[0].total_pendukung,
            total_pendukung_wanita: dataRes[0].total_pendukung_wanita,
            total_pendukung_pria: dataRes[0].total_pendukung_pria,
          });
          //});
          let dataPendukungDB = parseFloat(
            dataRes[0].total_pendukung / dataRes[0].target,
          );
          let dataRelawanDB = parseFloat(
            dataRes[0].total_relawan / dataRes[0].target,
          );
          self.state.dataRef.push(dataPendukungDB, dataRelawanDB);
        } else {
  ...
  GetDataDapil = uprove => {
    ...
    axios(config)
      .then(function (response) {
        ...
        if (response.data.success === true) {
          var objmaps64 = {};
          //console.log("dataReportTARGEETTTT",JSON.stringify(response.data.data));
  ...
  GetDataByGender = (id, dapilid) => {
    ...
    let urlapi;
    if (this.state.ishead === 1) {
      urlapi = API_URL + `/totalbygenderhead/` + id + `/` + dapilid;
    } else {
      urlapi = API_URL + `/totalbygender/` + id + `/` + dapilid;
    }
  ...
  getDataByUsia = id => {
    ...
    let urlapi;
    if (this.state.ishead === 1) {
      urlapi = API_URL + `/totalbyusiahead/` + id;
    } else {
      urlapi = API_URL + `/totalbyusia/` + id;
    }
    ...
    axios(config)
      .then(function (response) {
        let ft = [];
        if (response.data.success === true) {
          var dataUsiaDB = response.data.data;
          //var dataRev = dataUsiaDB.reverse();
          //console.log("Pilih usia MUDA: ",JSON.stringify(dataUsiaDB));
          self.state.dataUsiaGen.push(
            dataUsiaDB[0].muda,
            dataUsiaDB[0].dewasa1,
            dataUsiaDB[0].dewasa2,
            dataUsiaDB[0].dewasa3,
            dataUsiaDB[0].tua,
          );
          // (dataUsiaDB).map((items) => {
          //     self.state.datalabelsHarian.push(items.tgl)
          // })

          // (dataUsiaDB).map((items) => {
          //     self.state.dataHarian.push(items.jml);
          // })

          // self.setState({
          //     muda: dataUsiaDB[0].muda,
          //     dewasa1: dataUsiaDB[0].dewasa1,
          //     dewasa2: dataUsiaDB[0].dewasa2,
          //     dewasa3: dataUsiaDB[0].dewasa3,
          //     tua : dataUsiaDB[0].tua
          // })
        } else {
          console.log('errorrrrrrrita');
        }
  ...
  getDataHarianRekrut = id => {
    const self = this;
    var formData = new URLSearchParams();
    let urlapi;
    if (this.state.ishead === 1) {
      urlapi = API_URL + `/harianrekruthead/` + id;
    } else {
      urlapi = API_URL + `/harianrekrut/` + id;
    }
    var config = {
      method: 'get',
      url: urlapi,
    ...
    axios(config)
      .then(function (response) {
        let ft = [];
        if (response.data.success === true) {
          var dataHarianDB = response.data.data;
          //var dataRev = dataUsiaDB.reverse();
          //console.log("Pilih usia MUDA: ",JSON.stringify(dataHarianDB));
          //self.state.dataHarian.push(dataUsiaDB[0].muda, dataUsiaDB[0].dewasa1, dataUsiaDB[0].dewasa2,dataUsiaDB[0].dewasa3, dataUsiaDB[0].tua)

          dataHarianDB.reverse().map((items, index) => {
            let tgllbl = items.tgl.split('-');
            self.state.dataTanggalHarian.push(tgllbl[2] + '/' + tgllbl[1]);
            self.state.dataHarian.push(items.jml);
          });

          // (dataHarianDB).map((items, index) => {
          //     self.state.dataHarian.push(items.jml);
          // })

          //console.log("y90790", JSON.stringify(self.state.dataHarian));
        } else {
          console.log('errorrrrrrrita');
        }
      })
      .catch(function (error) {
        if (!error.response) {
          console.log('------ERORABYREKRUT' + error);
          //alert("ERROR KONEKSI: SILAHKAN COBA");
        } else {
          this.setState({
            loaderdatareport: false,
  ...
  render() {
    ...
    const dataHarianLine = {
      labels: this.state.dataTanggalHarian.reverse(), //["January", "February", "March", "April", "May", "June"],
      datasets: [
        {
          data: this.state.dataHarian.slice(-10).reverse(), //[20, 45, 28, 80, 99, 43, 21, 32, 33, 12],
        ...
        <ScrollView>
          <View>
            ...
            <View style={{top: -100}}>
              ...
              <View
                ...
                <View
                  ...
                  <Text
                    ...
                    {(
                      (parseInt(this.state.total_pendukung) /
                        parseInt(this.state.jmlTargetSuara)) *
                      100
                    ).toFixed(3)}
                    %
                  </Text>
              ...
              <View
                style={{marginLeft: 20, flexDirection: 'row', width: '80%'}}>
                <View
                  style={{
                    width: 55,
                    height: 25,
                  ...
                  <Text
                    style={{alignSelf: 'center', color: 'white', fontSize: 10}}>
                    {(
                      (parseInt(this.state.total_relawan) /
                        parseInt(this.state.jmlTargetSuara)) *
                      100
                    ).toFixed(3)}
                    %
            ...
            <View style={{marginTop: -80}}>
              ...
              <View
                ...
                <Text
                  ...
                  {this.state.total_pendukung + this.state.total_relawan} (
                  {(
                    ((this.state.total_pendukung + this.state.total_relawan) /
          ...
          <View>
            <View style={{width: '100%', marginLeft: 40, marginTop: 20}}>
              <Text>
                ...
                />
                Susunan Demografi Gender Pendukung
              </Text>
              <View
                style={{
                  width: '70%',
            ...
            <View
              ...
              <View style={{paddingLeft: 5}}>
                <Text
                  ...
                  {' '}
                  {this.state.total_pendukung_wanita} (
                  {(
                    (parseInt(this.state.total_pendukung_wanita) /
                      parseInt(
                        this.state.total_pendukung + this.state.total_relawan,
                      )) *
                    100
            ...
            <View
              ...
              <View style={{paddingLeft: 5}}>
                <Text
                  ...
                  {' '}
                  {this.state.total_pendukung_pria} (
                  {(
                    (parseInt(this.state.total_pendukung_pria) /
                      parseInt(
                        this.state.total_pendukung + this.state.total_relawan,
                      )) *
                    100
          ...
          <View style={{paddingLeft: 25}}>
            <View style={{width: '110%', marginLeft: 20, marginTop: 20}}>
              <Text>
                ...
                />
                {this.state.ishead === 1
                  ? 'Susunan Demografi Per Manager'
                  : 'Sususan Demografi Wilayah Kecamatan'}
              </Text>
              <View
                style={{
                  width: '70%',
            ...
            <View
              ...
              <View
                ...
                <Text style={{fontWeight: 'bold'}}>
                  {this.state.ishead === 1 ? 'Kota' : 'Kecamatan'}
                </Text>
              </View>
              <View
                style={{
            ...
            <View style={{width: '100%', marginLeft: 20, marginTop: 20}}>
              <Text>
                <Icon
                  ...
                  style={{paddingRight: 10}}
                />
                Sususan Demografi Berdasarkan Usia
              </Text>
              <View
                style={{
                  width: '70%',
          ...
          <View style={{marginBottom: 50, marginRight: 10}}>
            {this.state.dataHarian.length > 0 ? (
              <LineChart
                data={dataHarianLine}
                width={screenWidth}
                height={306}
