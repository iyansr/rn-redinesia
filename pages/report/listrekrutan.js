...
export default class Listpendukung extends Component {
  ...
  componentDidMount() {
    this._unsubscribe = this.props.navigation.addListener('focus', () => {
      const {refnum} = this.props.route.params;
  ...
  ProfilesData = async () => {
    ...
    try {
      ...
      if (value !== null) {
        ...
        this.GetDataRegister(this.state.refNumber);
      }
    } catch (e) {
      // error reading value
    }
  ...
  GetDataRegister = hp => {
    ...
    const self = this;
    //console.log("8098079790", hp);
    var formData = new URLSearchParams();
    var config = {
      method: 'get',
      url: API_URL + `/userdukungcalegview/` + hp,
      //data : formData,
      headers: {
  ...
  onRefresh() {
    ...
    this.setState({isFetching: true}, () => {
      this.GetDataRegister(this.state.uname);
    });
  ...
  render() {
    ...
    const filteredData = this.state.keyword
      ...
      : this.state.dataRef;
    // console.log("dddd", JSON.stringify(this.state.dataRef));
