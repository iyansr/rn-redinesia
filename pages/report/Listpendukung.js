...
export default class Listpendukung extends Component {
  ...
  ListPendukungAsync = async () => {
    try {
      const value = await AsyncStorage.getItem('@storage_pendukung_Key');
      if (value !== null) {
        const datastring = JSON.parse(value);
  ...
  GetDataPendukung = idk => {
    // const storeDataListPendukung = async (value) => {
    //     try {
    //         await AsyncStorage.setItem('@storage_pendukung_Key', JSON.stringify(value))
    //     } catch (e) {
    //         // saving error
    //     }
    // }
    ...
    axios(config)
      .then(function (response) {
        ...
        if (response.data.success === true) {
          let listdatapendukung = [];
          response.data.data.forEach((items, index) => {
            //console.log("----datetime1----", items.user_id);
          ...
          //storeDataListPendukung(listdatapendukung)
        } else {
  ...
  onRefresh() {
    this.setState({isFetching: true}, () => {
      this.GetDataPendukung(this.state.kandidat_id);
    });
          ...
          <TouchableOpacity
            onPress={() =>
              this.props.navigation.navigate('DashboardNavigator', {
                screen: 'personapendukungdetail',
                params: {idrelawan: item.id},
              })
            }>
            <Text
              style={{
