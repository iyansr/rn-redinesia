...
class Takecamera extends Component {
    ...
    componentDidMount() {
        this._unsubscribe = this.props.navigation.addListener('focus', () => {
            const { backpage, iduser } = this.props.route.params;
            //console.log("wedsdsdsds", iduser);
            this.setState({
                backpageconf : backpage,
                iduserdetail : iduser
            })
        });
    }
    ...
    takePicture = async () => {
        ...
        const storeDataKTP = async (value) => {
            try {
                await AsyncStorage.setItem('@storage_FotoRekrut_Key', JSON.stringify(value))
            } catch (e) {
                // saving error
            }
        }
        ...
        if (this.camera) {
            ...
            storeDataKTP(assetsrender);
            if(this.state.backpageconf === 1){
                this.props.navigation.navigate("buktifoto")             
            }else{
                this.props.navigation.navigate("editrekrutdata", {useridrelawan: this.state.iduserdetail})  
                //this.props.navigation.navigate("buktifoto")             
            }
    ...
    takeimagegallery = async () => {
        ...
        if(this.state.backpageconf === 1){
            ...
            this.props.navigation.navigate("editrekrutdata", {useridrelawan: this.state.iduserdetail})  
            //this.props.navigation.navigate("editrekrutdata")             
        }
    }
