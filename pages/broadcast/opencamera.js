...
class Takecamera extends Component {
    ...
    componentDidMount() {
        this._unsubscribe = this.props.navigation.addListener('focus', () => {
            const { idtarget } = this.props.route.params;
            //console.log("TTTT",idtarget);
            this.setState({
                target: idtarget
            })
    ...
    takePicture = async () => {
        ...
        const storeDataKTP = async (value) => {
            try {
                await AsyncStorage.setItem('@storage_BroadcastFoto_Key', JSON.stringify(value))
            } catch (e) {
                // saving error
            }
        }
        ...
        if (this.camera) {
            ...
            this.props.navigation.navigate("sendbroadcast", {idfrom : "camera"})             
        }
    };
