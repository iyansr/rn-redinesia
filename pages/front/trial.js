...
class AppLanding extends Component {
      ...
      .then(function (response) {
        ...
        if (response.data.success === true) {
          ...
          response.data.data.map((items, index) => {
            ...
            //     selectedValueDistrict: ""
            // })
            //self.GetDataCity(items.prov_id);
            //}
          });
      ...
      .then(function (response) {
        ...
        if (response.data.success === true) {
          response.data.data.map((items, index) => {
            ...
            //     selectedValueDistrict: ""
            // })
            //self.GetDataDistrict(items.city_id);
          });
          self.setState({
            selectCity: [
  ...
  GetDataPartai = () => {
    ...
    axios(config)
      .then(function (response) {
        //console.log("---->pt"+JSON.stringify(response.data.data));
        if (response.data.success === true) {
          let nomor;
          response.data.data.map((items, index) => {
            //console.log(">>>>>>city",items['city_name'], items["city_id"]);
            ...
            //if(index == items["city_id"]){
            index = items['pt_id'];
            nomor = items['pt_nomor'];
            //console.log(">>>>>>districtddd",index+"-"+items["dis_id"]+"-"+items["dis_name"]);
            self.state.selectPartai[nomor] = nomor + ' - ' + items['pt_nama'];
            //}
          });
          // self.setState({
          //     selectPartai: [...self.state.selectPartai.filter(function (e) {return e != null;})]
          // });
          //console.log("+++++++CITYH"+JSON.stringify(self.state.selectCity.filter(function (e) {return e != null;})));
          //console.log("+++++++ssssbbb"+JSON.stringify(self.state.selectDistrict));
        } else {
  ...
  registrationProcess = () => {
    ...
    formData.append('referal', '081510646400');
    formData.append('partai', self.state.selectedValuePartai);
    formData.append('dapil', self.state.dapil);
    //formData.append('idaccount',  "");
    formData.append('status', 0);
    formData.append('owner', '081510646400');
          ...
          <View style={styles.formcontainer}>
            ...
            <View style={styles.action1}>
              ...
              <TextInput
                ...
                onChangeText={text => this.setState({dapil: text})}
              />
              {
                // errors.named &&
                //     <Text style={styles.error}>{errors.named}</Text>
            ...
            <View style={styles.action1}>
              ...
              <SelectDropdown
                ...
                buttonTextStyle={{textAlign: 'left', flex: 1, fontSize: 15}}
                defaultButtonText={
                  this.state.selectedPartai != ''
                    ? this.state.selectedPartai
                    : 'Pilih Partai'
                }
                dropdownStyle={{height: 180, fontSize: 12}}
                disabled={this.state.selectPartai.length > 0 ? false : true}
                rowTextStyle={{textAlign: 'left'}}
                onSelect={(selectedItem, index) => {
                  //console.log("====--====",selectedItem, index)
                  this.setState({
                    selectedPartai: selectedItem.split(' - ')[0],
                    selectedValuePartai: selectedItem.split(' - ')[0],
                  });
                }}
                buttonTextAfterSelection={(selectedItem, index) => {
                  return selectedItem;
                ...
                defaultValue={this.state.selectedPartai}
              />
              {
            ...
            <View style={styles.action1}>
              ...
              <SelectDropdown
                ...
                dropdownStyle={{height: 190, fontSize: 12}}
            ...
            <View style={styles.action1}>
              ...
              <SelectDropdown
                ...
                rowTextStyle={{textAlign: 'left'}}
                onSelect={(selectedItem, index) => {
                  this.GetDataDistrict(selectedItem.split(' - ')[0]);
                  //console.log("====--====",selectedItem, index)
                  this.setState({
                    selectDistrict: [],
                    selectedDistrict: '',
                  ...
                  this.setState({
                    selectedCity: selectedItem.split(' - ')[0],
                    selectedValueCity: selectedItem.split(' - ')[0],
                  });
                }}
                buttonTextAfterSelection={(selectedItem, index) => {
                  //console.log("--> after select"+selectedItem, index);
            ...
            {/* <View style={styles.action1}>
                        <View style={styles.lefticon}>
                        { this.state.loadDis ? 
                            <UIActivityIndicator color='#000000' zIndex={999999} size={19} /> 
                        ...
                        <SelectDropdown
                                ...
                                buttonTextStyle={{textAlign: 'left', flex: 1, fontSize: 15}}   
                                defaultButtonText={this.state.selectedDistrict != "" ? this.state.selectedDistrict : "Pilih Kecamatan"}     
                                dropdownStyle={{height: 180, fontSize: 12}}
            ...
            {/* <View style={styles.action1}>
                        ...
                        <SelectDropdown
                                ...
                                buttonStyle={{width: '88%',  
                                    ...
                                    borderColor: '#828282',paddingHorizontal: 0, backgroundColor: '#FFFFFF'}}
                                buttonTextStyle={{textAlign: 'left', flex: 1, fontSize: 15}}   
                                defaultButtonText={this.state.selectedSubDistrict != "" ? this.state.selectedSubDistrict : "Pilih Kelurahan"}     
                                dropdownStyle={{height: 200, fontSize: 12}}
                                disabled={this.state.selectSubDistrict.length > 0 ? false : true}
                                rowTextStyle={{textAlign: 'left'}}
                                onSelect={(selectedItem, index) => {
                                    //console.log("====--====",selectedItem, index)
                                    this.setState({
