...
class Takecamera extends Component {
    ...
    takePicture = async () => {
        ...
        const storeDataSelfi = async (value) => {
            try {
                await AsyncStorage.setItem('@storage_SELFI_Key', JSON.stringify(value))
            } catch (e) {
                // saving error
            }
        }
        ...
        if (this.camera) {
            ...
            storeDataSelfi(assetsrender);
            this.props.navigation.navigate("updatedok")             
        }
    };
