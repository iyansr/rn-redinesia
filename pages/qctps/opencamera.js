...
class Takecamerasurvey extends Component {
    ...
    componentDidMount() {
        this._unsubscribe = this.props.navigation.addListener('focus', () => {
            const { idqc } = this.props.route.params;
            this.setState({ 
                idquickcount: idqc
            });
    ...
    takePicture = async () => {
        ...
        const storeDataKamera = async (value) => {
            try {
                await AsyncStorage.setItem('@storage_QC_Key', JSON.stringify(value))
            } catch (e) {
                // saving error
            }
        }
        ...
        if (this.camera) {
            ...
            if(this.state.idquickcount > 0){
                this.props.navigation.navigate("editqc", {idqc: this.state.idquickcount, pscreen: "camera"})             
            }else{
                this.props.navigation.navigate("inputqc")             
            }
        }
    };
