...
export default class Detailcreen extends Component {
  ...
  ProfilesData = async () => {
    ...
    try {
      ...
      if (value !== null) {
        ...
        this.GetDataRelawan(datastring[0].user_id);
      } else {
        this.setState({
          session_id: null,
        });
  ...
  ImageData = async () => {
    const self = this;
    try {
      const value = await AsyncStorage.getItem('@storage_FotoProfile_Key');
      if (value !== null) {
        const datastring = JSON.parse(value);
    ...
    axios(config)
      .then(function (response) {
        ...
        if (response.data.success === true) {
          alert('Sukses');
          //self.simpanSelfi();
          navigation.navigate('DashboardBottomNavigator', {screen: 'Profile'});
        } else {
          alert(response.data.message);
        }
      })
        ...
        <ScrollView style={{marginTop: 0, marginLeft: 5}}>
          <View style={styles.formcontainer}>
            <View
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate('bukakameraprofile', {
                    backpage: 2,
