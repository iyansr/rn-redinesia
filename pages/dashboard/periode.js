...
export default class Listrelawan extends Component {
  ...
  render() {
    ...
    const renderItem = ({item}) => (
      // <TouchableOpacity keyExtractor={(items, index) => index.toString()} onPress={() => item.periode === 1 ? this.props.navigation.navigate("DashboardNavigator", {screen : 'listsurvey', params: {idperiode: item.id}}) : alert("Periode Ini Belum Aktif")} style={{width: '95%', height: 60, flexDirection: 'row', marginBottom: 10, borderBottomColor: '#fcba03', borderBottomWidth: 1}}>
      <TouchableOpacity
