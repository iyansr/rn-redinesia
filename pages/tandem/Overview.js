...
export default class Overview extends Component {
  ...
  componentDidMount(props) {
    // this._unsubscribe = this.props.navigation.addListener('focus', (props) => {
    const {idkandidat} = this.props;
    //console.log(JSON.stringify(this.props));
    //console.log("---->ssss", this.props.idkandidat);
    this.setState({
    ...
    try {
      ...
      if (value !== null) {
        ...
        if (datastring.length > 0) {
          ...
          this.getDataByUsia(idkandidat);
          this.getDataHarianRekrut(idkandidat);
          this.GetDataPendukung(idkandidat);
          this.GetDataDapil(idkandidat);
          this.GetDataByGender(idkandidat, datastring[0]['dapil_id']);
        }
      }
    } catch (e) {
      // error reading value
  ...
  GetDataPendukung = idkandidat => {
    ...
    var config = {
      method: 'get',
      url: API_URL + `/totalpendukung/` + idkandidat,
      data: formData,
      headers: {
  ...
  GetDataByGender = (id, dapilid) => {
    ...
    var config = {
      method: 'get',
      //url: API_URL+`/totalbygender/`+id+`/`+dapilid,
      url: API_URL + `/totalbygenderdetailhead/` + id,
      headers: {
        //'Authorization': 'Basic c3RldmVuOm9rZWJhbmdldA==',
        Authorization: 'Bearer ' + self.state.token,
      },
      ...
      .then(function (response) {
        ...
        if (response.data.success === true) {
          ...
          dataHarianDB.map((items, index) => {
            self.state.dataTanggalHarian.push(items.tgl);
            self.state.dataHarian.push(items.jml);
          });
          ...
          <View>
            <View style={{width: '100%', marginLeft: 40, marginTop: 20}}>
              <Text>
                ...
                />
                Sususan Demografi Gender Pendukung
              </Text>
              <View
                style={{
                  width: '70%',
